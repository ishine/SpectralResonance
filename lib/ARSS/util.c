...
int32_t gettime()	// in milliseconds
{
 ...
 double roundoff(double x)	// nearbyint() replacement, with the exception that the result contains a non-zero fractional part
{
	if (x>0)
		return x + 0.5;
	else
...
float getfloat()
{
 ...
 int32_t smallprimes(int32_t x)	// returns 1 if x is only made of these small primes
{
	int32_t i, p[2]={2, 3};
 ...
 int32_t nextsprime(int32_t x)	// returns the next integer only made of small primes
{
