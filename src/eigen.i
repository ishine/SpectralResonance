...
/*
It will not work across modules...
%typemap(out) Eigen::MatrixXf
{
    $result = $1.matrix;
}

%typemap(in) Eigen::MatrixXf
{
    $1 = Eigen::MatrixXf($input.matrix);    
}

%typemap(out) Eigen::MatrixXd
{
    $result = Eigen::MatrixXd($1.matrix);    
}

%typemap(in) Eigen::MatrixXd
{
    $1 = Eigen::MatrixXd($input.matrix);    
}
*/
%include "Eigen.h"
%include "Base.h"
...
Eigen::MatrixXf to_matrixf(const SimpleEigen::Matrix<float> & m) {
    return Eigen::MatrixXf(m.matrix);
}
Eigen::MatrixXd to_matrixd(const SimpleEigen::Matrix<double> & m) {
    return Eigen::MatrixXd(m.matrix);
}

SimpleEigen::Matrix<float>  from_matrixf(const Eigen::MatrixXf& m) {
    SimpleEigen::Matrix<float> mat;
    mat.matrix = m;
    return mat;
}
SimpleEigen::Matrix<double> from_matrixd(const Eigen::MatrixXd& m) {
    SimpleEigen::Matrix<double> mat;
    mat.matrix = m;
    return mat;
}
%}
